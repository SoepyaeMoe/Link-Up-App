<template>
    <div class="bg-gray-200 md:flex">
        ...
        <div class="w-[100%] h-screen overflow-y-scroll scrollbar-hide relative mt-10 md:mt-0">
            <div :class="{ 'hidden': loadingContentDetail }" class="w-[100%]">
                <img class="mx-auto" src="@/assets/spinner.svg" alt="">
            </div>

            <div :class="{ 'hidden': !loadingContentDetail }"
                class="pt-5 mx-auto md:w-[70%] md:px-0 px-2 w-[100%] mt-5">
                <Back></Back>
                <div class="px-4 py-4 shadow-md bg-gray-100 mb-5 rounded-md">
                    <!-- content header -->
                    <div class="flex relative">
                        <img :src="contentDetail.user_photo ? profile_img_path + contentDetail.user_photo : 'https://ui-avatars.com/api/?name=' + contentDetail.user_name"
                            alt="" class="rounded-full w-14">
                        <div class="ms-3 mt-1">
                            <router-link :to="'user-profile/' + contentDetail.content_user_id"
                                class="font-semibold mb-0 block">
                                {{ contentDetail.user_name }}
                            </router-link>
                            <small class="p-0">{{ contentDetail.created_at }}</small>
                        </div>

                        <img @click="openOption" class="absolute top-2 right-2 hover:cursor-pointer" id="more"
                        ...
                        <div id="options"
                            ...
                            <ul class="py-2">
                                ...
                                <!-- follow and unfollow -->
                                <li v-if="!contentDetail.follow_status && contentDetail.content_user_id != authUser.id"
                                    @click="follow(contentDetail.content_user_id)"
                    ...
                    <div class="mt-5 max-h-[200px] overflow-hidden">
                        <h3 class="font-semibold mb-2 text-lg">{{ contentDetail.title }}</h3>
                        <p class="">{{ contentDetail.content }}</p>
                    </div>
                    ...
                    <div class="mt-2 max-h-[500px] overflow-hidden">
                        <img id="myImg" @click="viewImg" :src="content_img_path + contentDetail.content_image"
                            class="mx-auto" alt="">
                    </div>
                    ...
                    <div class="mt-3 flex">
                        <div class="me-3">
                            <i class='bx bx-heart text-xl hover:cursor-pointer'
                                v-show="contentDetail.react_status == false"
                                @click="sendReact({ 'content_id': contentDetail.id, 'source_content': 'contents' })"></i>
                            <i class='bx bxs-heart text-red-600 text-xl hover:cursor-pointer'
                                @click="sendReact({ 'content_id': contentDetail.id, 'source_content': 'contents' })"
                                v-show="contentDetail.react_status == true"></i>
                            {{ contentDetail.react }}
                        </div>
                        <div @click="seeComment(contentDetail.id, contentDetail.user_id, contentDetail.user_name)">
                            <i class='bx bx-comment text-xl hover:cursor-pointer'></i>
                            <span class="text-gray-700">
                                {{ contentDetail.comment_count }}
                            </span>
                        </div>
                    </div>
                    <!-- end footer -->
            ...
            <!-- comments modal box -->
            <div class="hidden w-[100%] h-[100%] px-2 z-50 md:top-0 top-3 right-0"
                style="background-color: rgba(0,0,0,0.5)" id="comment_modal">
                <div class="bg-gray-50 md:w-[60%] h-[550px] mx-auto mt-10 rounded-md shadow overflow-hidden">
                    <div class="border border-bottom-gray-300 bg-gray-100 relative">
                        <h1 class="text-center font-semibold text-xl my-2">Soe Pyae Moe's Post</h1>
                        ...
                        <div v-if="comments[content_id] != undefined">
                            <div v-for="comment in comments[content_id]" :key="comment.id">
                                <div class="flex mb-2">
                                    ...
                                    <div class="bg-gray-200 ms-2 rounded-md pb-4 pl-2 pr-8 relative">
                                        ...
                                        <router-link :to="'user-profile/' + comment.user.id"
                                            class="font-semibold text-sm">{{ comment.user.name }}</router-link>
                                        <p class="text-sm">{{ comment.comment }}</p>
                                        <p class="text-[8px] text-gray-500 absolute right-1 buttom-0">
                                            {{ comment.updated_at }}</p>
                                        <!-- comment option -->
                                        <div
                                            class="absolute shadow bg-gray-50 top-5 right-[-10px] hidden options rounded">
...
export default {
    ...
    methods: {
        ...
        ...mapActions(['getComment', 'sendComment', 'sendReact']),
        seemore(e) {
    ...
    computed: {
        ...mapGetters(['contentDetail', 'comments', 'otherUserInfo', 'authUser', 'contents']),
    },
    mounted() {
        ...
        // this.getContentDetail(this.$route.params.id);
        this.$axios.get(`content-detail/${this.$route.params.id}`)
            .then(response => {
                if (response.data.status == 200) {
                    this.$store.commit('setContentDetail', response.data.data);
                    this.loadingContentDetail = true;
                }
            })
    },
}
